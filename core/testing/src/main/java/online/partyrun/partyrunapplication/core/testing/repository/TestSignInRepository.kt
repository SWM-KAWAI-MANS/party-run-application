...
class TestSignInRepository : SignInRepository {
    private val tokenSet = MutableSharedFlow<ApiResponse<SignInTokenResponse>>(replay = 1, onBufferOverflow = BufferOverflow.DROP_OLDEST)

    /**
     * 테스트용 데이터를 방출하는 메소드
     */
    suspend fun emit(value: ApiResponse<SignInTokenResponse>) {
        tokenSet.emit(value)
    }

    override suspend fun signInGoogleTokenToServer(idToken: GoogleIdToken): Flow<ApiResponse<SignInTokenResponse>> = tokenSet
}